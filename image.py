...
def create_image(a):
    ...
    img.save('static/img/main.jpg')
    img = Image.open('static/img/main.jpg')
    ...
    response_coords = get(f"{a}/coords")
    ...
    with Image.open('static/img/main.jpg') as img:
        draw = ImageDraw.Draw(img)
        for u in range(4):
            for e in range(4):
                for i in range(len(ans[u][e])):
                    for j in range(len(ans[u][e])):
                        draw.rectangle(((j + 64 * e, i + 64 * u), (j + 64 * e * 2, i + 64 * u * 2)),
                                       fill=(ans[u][e][i][j], ans[u][e][i][j], ans[u][e][i][j]), width=1)
        img.save('static/img/main.jpg')
